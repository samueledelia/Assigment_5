...
delta2 = 10
notional = value_portfolio_hs
n_principal_component = 6
...
returns_3 = shares_3 / shares_3.shift(delta2)
returns_3 = returns_3[1:]
x_3 = np.log(returns_3)

index = np.arange(delta2, len(portfolio3_df), delta2)
x_3 = x_3.iloc[index]
n3 = x_3.shape[1]
weights_3 = np.ones(n3) / n3
x_3 = x_3.to_numpy()
weights_3 = np.reshape(weights_3, (returns_3.shape[1]))

var_pca = []
for i in range(n_principal_component):
    var_pca_i = utilities.pca_var_es(x_3, weights_3, i+1, delta2, alpha, notional)
    var_pca = np.concatenate((var_pca, var_pca_i), axis=0)
...
print("Value at Risk (VaR): ", var_pca)
#print("Expected Shortfall (ES): {:.5f}".format(0))
print("Plausibility Check: {:.5f}".format(var_check_pca[0]))
